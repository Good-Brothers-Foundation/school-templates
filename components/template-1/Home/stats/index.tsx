...
function StatCard({ s }: { s: (typeof stats)[number] }) {
  ...
  return (
    <div
      className={`group relative overflow-hidden rounded-4xl ${s.bg} p-8 shadow-soft hover-lift ${s.span}`}
    >
      {s.label.toLowerCase() !== "happy students" ? (
        <div
          className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-white/30 blur-2xl"
          aria-hidden
        />
      ) : (
        <div className="absolute right-10 bottom-0 h-[95%] aspect-14/16 rounded-full ">
          <Image
            src={"/template-1/stats/hero-1.png"}
            alt="student image "
            fill
            objectFit="contain"
          />
      ...
      <div className="relative flex h-full flex-col justify-between gap-6">
        <div
          className={`grid h-14 w-14 place-items-center rounded-2xl bg-white ${s.text} shadow-soft`}
        >
      ...
      <Wrapper>
        <div className="reveal mx-auto mb-12 max-w-2xl text-center">
          ...
          <p className="mt-4 text-muted-foreground">
            A decade of nurturing curiosity, kindness, and confidence.
          </p>
        </div>
